...
bool prompt_date_field(const wchar_t* title, Date* date, const Date* current = NULL)
{
    ...
    if (!prompt_int_field(title, L"请输入年份（例如：2024）", &year, year)) return false;
    if (!prompt_int_field(title, L"请输入月份（1-12）", &month, month)) return false;
    if (!prompt_int_field(title, L"请输入日期（1-31）", &day, day)) return false;
    date->year = year;
    date->month = month;
    date->day = day;
    return true;
...
bool prompt_violation_field(const wchar_t* title, ViolationLevel* level, ViolationLevel current = VIOLATION_NONE)
{
    ...
    if (!prompt_int_field(title, L"请输入违章等级：0 无，1 较轻微，2 轻微，3 中等，4 较严重，5 严重", &value, value)) return false;
...
void draw_status(const AppContext* ctx)
{
    ...
    _snwprintf_s(status, 128, _TRUNCATE, is_logged_in(ctx) ? L"当前用户：%s" : L"当前用户：未登录", is_logged_in(ctx) ? user : L"");
...
void draw_cars(const AppContext* ctx, const GuiState* state, const Button* buttons, int count)
{
    ...
    if (line_count == 0) {
        wcsncpy_s(buffer[0], L"无车辆记录", _TRUNCATE);
        lines[0] = buffer[0];
        line_count = 1;
    }
...
void draw_insurance(const AppContext* ctx, const GuiState* state, const Button* buttons, int count)
{
    ...
    if (state->insurance_view == VIEW_POLICY_LIST) {
        ...
        if (line_count == 0) {
            wcsncpy_s(buffer[0], L"无保单记录", _TRUNCATE);
            lines[0] = buffer[0];
            line_count = 1;
        }
        draw_rows(lines, line_count);
        ...
        if (line_count == 0) {
            wcsncpy_s(buffer[0], L"无理赔记录", _TRUNCATE);
            lines[0] = buffer[0];
            line_count = 1;
        }
        draw_rows(lines, line_count);
...
void handle_add_car(AppContext* ctx, GuiState* state)
{
    ...
    if (!prompt_violation_field(L"添加车辆", &violation)) return;
    if (!prompt_date_field(L"添加车辆", &date)) return;
...
void handle_home_action(AppContext* ctx, GuiState* state, int action, bool* running)
{
    ...
    case HOME_CARS: state->screen = SCREEN_CARS; state->car_show_mine = false; state->car_page = 0; set_message(state, L"已进入车辆管理"); break;
...
void handle_car_action(AppContext* ctx, GuiState* state, int action)
{
    ...
    case CAR_LIST_ALL: state->car_show_mine = false; state->car_page = 0; set_message(state, L"显示全部车辆"); break;
    case CAR_LIST_MINE: state->car_show_mine = true; state->car_page = 0; set_message(state, L"显示我的车辆"); break;
